...
# 1. App configuration
# ------------------------------------------------------------------------------
st.set_page_config(page_title="YONG's SUDOKU", page_icon="🏄", layout="centered")

st.markdown(
    """
    <style>
        html { scroll-behavior: smooth; }
        .stApp { max-width: 100%; padding-left: 0.5rem; padding-right: 0.5rem; overflow-x: hidden; }
        ...
        /* ---------------------------------------------------------------
           9x9 판독 결과 격자 스타일링
           - Streamlit 버전마다 컬럼의 data-testid 값이 "column" / "stColumn"
             등으로 다를 수 있어, 속성 이름에 의존하지 않고 "가로줄의 직계
...
st.markdown('<h1 class="app-title">🏄 YONG's SUDOKU</h1>', unsafe_allow_html=True)
